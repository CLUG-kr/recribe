...
<body>
        ...
        <div class="col-lg-5 mt-5 mt-lg-0 d-flex align-items-center" style="margin:0px auto">
			<form class="php-email-form" action="${pageContext.request.contextPath}/signUpOk">
				...
				<div class="form-group col-md-6" style="float:left;padding-left:20px;">
					<button class="signUp" id="signUp">임시비밀번호 발송</button><br>
				</div>
			</form>
        </div>
  ...
  var sendPasswordBtn = function sendPassword(e) {
		...
		if ($("#email1").val() != "" && $("#email2").val() != "") {
			...
			$.ajax({
				...
				success: function success(data) {
					...
					var sendPassword = data.sendPassword;
					
					if(isEmailExist) {
						if(updatePassword && sendPassword){
							alert("임시 비밀번호를 이메일로 발송하였습니다. 로그인 하신 후 비밀번호를 꼭 변경해주세요.");
						}else{
							alert("오류가 발생했습니다. 자세한 사항은 문의 바랍니다.");			
						}
					} else {
						alert("존재하지 않는 이메일 입니다. 이메일 주소를 다시 입력해주세요.");
					}
				},
				error: function() {
	...
	$("#sendPasswordBtn").click(sendPasswordBtn);
